...
- **記憶**(order 30)— 智能記憶模塊(**嚴格按 session 隔離**:不再混入其他 session 的內容[v0.10.0 起;舊「同 cwd 聚合」行為保留為 `/api/memories?scope=project` 白名單;`mem_save` 新寫入會蓋上 session id 戳記]+ 記憶活動四透鏡
...
外加:提示詞優化器(composer「優化」按鈕)、洞察浮窗 FAB(夜間模式可讀、可拖動)、**洞察面板可自由調整大小**(拖右緣/下緣/右下角,尺寸跨刷新保持)、正文一律**原生 Markdown 渲染**(觀測敘事/記憶模塊/自動洞察/價值總結/洞察對話,與主對話同一 `MarkdownText` 管線)。
...
重啟 web 後生效。(開發期間可將依賴指向本地目錄:`file:/path/to/dsh-insights`。)
